...
Version 6 of Raptor now finally has an integrated looper facility. There are two related GUI controls, a numbox-toggle pair labeled `loop`, at the bottom of the `panel` subpatch. The numbox tells Raptor the number of bars to record. Engaging the toggle saves whatever you just played (up to the given number of bars, but the looper will also be happy with less) to an internal buffer and immediately starts looping it, indicated by briefly flashing the pulse indicator in the `loop` subpatch at the beginning of each iteration. Disengaging the toggle instantly clears the loop and resumes normal arpeggiator operation. Note that while the loop is playing, note input to the arpeggiator is suspended, so you can use your input controller to play along. Control input works normally, though, so you can also start twiddling the knobs, change presets, etc.

Loops are always quantized to whole bars, so you *must* keep playing for at least one bar before pressing the `loop` button to have anything recorded. This kind of "launch quantization" should be well familiar to Ableton Live users. The main difference here is the 1-button style of operation: The looper is *always* listening, so once transport is rolling *and* you actually start playing, you just push the button to loop whatever you just played, rather than requiring a separate button press to explicitly start the recording.

Another numbox-toggle pair labeled `load/save` can be found in the `loop` subpatch, which lets you save the loop that is currently playing to disk and reload it later. When a loop is playing, the `load/save` button turns red to indicate that you can save the loop to a file by pressing it. The file is saved in the preset folder under the name of the preset that is currently selected, with the number in the numbox (0..99) and the extension `.loop` tacked onto it, and can be reloaded later by pressing the `load/save` button again when no loop is playing. Both the `loop` and the `load/save` button also have PACER bindings, so they can be operated while your hands remain on your input device.
