...
Using the patch is easy enough, however. Hook up your MIDI keyboard and synthesizer to Pd's MIDI input and output, respectively, press `play` and just play some chords. I'm mostly using Raptor with a MIDI guitar and a [Nektar PACER][] foot pedal these days, so the patch is somewhat tailored for that use case, but it also works fine with just a MIDI keyboard and a free software synth such as [Qsynth][]. You can have a whole band of Raptors accompanying you, by running multiple instances of the main patch in concert in the same Pd instance. Just make sure that you select one of the instances as the time master (`M` toggle in the `time` subpatch), which happens automatically if you press the big green `play` toggle in one of the instances.

This project is still work in progress. There's some stuff on the TODO list, such as OSC control and a looper facility. The included presets also need some work, but hey, you get them for free, and you can always change them! ;-) Bug reports and other contributions are welcome. And I'm always curious about music made with my programs, so please do send me links to music produced with Raptor!
...
Raptor's note generation process is guided by the chosen *meter* which determines the pulse strength of each step in the pattern. We employ Barlow's *indispensability* measure to calculate these. Barlow's method requires the meter (or more precisely the number of beats) to be specified in *stratified* form as a list of (prime) subdivisions, such as 3-2-2 (e.g., 3/4 subdivided into 16th notes), 2-3-2 (6/8 in 16ths), or 2-2-3 (12/16). Therefore, if you to specify the meter as a single composite number, Raptor assumes a partition of the number into its prime factors in ascending order, e.g.: 6 = 2-3, 12 = 2-2-3, 15 = 3-5, etc. This is usually in line with musical tradition (at least in the simple cases), but if you want a different stratification, you can also specify it explicitly as a Pd list such as `3 2`, `2 3 2`, `5 3`, etc. In any case, Raptor suggests a note value when you specify a meter, by rounding up the number of beats to the next power of 2 (so, e.g., 4 becomes 4/4, 6 becomes 6/8, 15 becomes 15/16, etc.). If the suggested default isn't what you want, you can also enter the desired value in the number box at the bottom of the `meter` subpatch.

*Tempo* may be specified using the traditional quarter beats per minute (bpm) value. The corresponding pulse period in ms is calculated automatically and displayed in the numbox labeled `ms`. If you prefer, you can also enter the period and have the corresponding bpm value calculated instead. Note that the pulse length depends on the note value of the meter, which is usually a power of 2. E.g., assuming a tempo of 120 bpm, quarters run at 500 ms per step, 8ths at double speed (250 ms/step), 16ths at quadruple speed (125 ms/step), etc. 

Raptor can sync to an external time source via MIDI clock messages, in which case that time source determines tempo and pulse period. Most DAWs support this, you just need to enable MIDI clock output in the DAW and connect it to any of Pd's MIDI inputs. Once Raptor starts receiving MIDI clocks, the pulse button in the `time` patch turns red to indicate MIDI clock input, playback starts automatically, and the current tempo and pulse length are displayed in the tempo section. Playback stops as soon as Raptor receives a MIDI stop message, or hasn't received MIDI clocks for some time (3 seconds in the current implementation, but this value can be changed in the `midiclock` abstraction). **NB:** Raptor's MIDI clock sync is a bit rudimentary right now, in particular, it lacks support for song position pointer (SPP) messages. To get a proper sync, you need to start playback in the DAW at the beginning of a bar, and have the meter set correctly in Raptor.
...
- *PC* refers to "program change", *CC* to "control change" messages. Like any MIDI input in Raptor, these are *only* received on the MIDI input channel set in the panel (using the numbox labeled`in`). If you only ever use a single Raptor instance, then it's best to have this set to zero to denote "omni", so that PC and CC messages will be received no matter what MIDI channel they're on. However, if you run multiple Raptor instances (or other instruments) which need to be controlled using various input controllers on different MIDI channels, then omni mode is most likely off the table, and you'll have to configure the input channels accordingly. This may cause the rather annoying issue that a preset selected by PC appears to be "stuck" if it changes the input channel, thus blocking further PC messages from the same device. Since Raptor doesn't have a separate channel for control input, there's no easy way around this catch-22 in the current implementation, and you'll probably need to use the GUI to change the preset or adjust the input channel in such situations.
- The table also lists the range of parameters. For "discrete" controls (0-1 switches: hold, mute, raptor mode, and uniq; 0-n switches: mode), the control value is taken as is, and any value greater than the number of alternatives is clamped to the given range. Thus, for the 0-1 switches, *any* positive value means "on". For other ("continuous") controls the full 0-127 MIDI range is mapped to the parameter range, and a CC value of 64 denotes the middle of the range (which means zero for parameters with a bipolar range, such as the "bias" parameters explained below).
...
Talking about controls, let's have a quick look at some important GUI controls that you should know about. As indicated below, some of these are saved in the raptor6 patch itself, so you can just save the patch to change their defaults.

First, in the main patch there's the `mod-switch` control (saved in the patch) which assigns some PACER controls to various different parameters for hands-free operation, please check the pacer subfolder for details. Most related controls can be found in the `panel` subpatch; the most prominent of these are the yellow mute toggle (labeled `M`) which mutes the arpeggiator (stops all note output), and the green hold toggle (labeled `H`) which holds all note input in memory, so that the arpeggiator keeps playing the same pattern while this toggle is engaged. These are intended mostly for live control and automation, so they both have MIDI bindings (CC64 and CC67). Of course, you can also operate them in the GUI, but their state is neither saved in the patch nor in presets.

Second, in the `time` subpatch there is the time master (`M`) toggle (saved in the patch) that we already discussed. There's also a second (unlabeled) start/stop toggle (not saved) which enables you to start and stop the sequence manually, and a button (a Pd "bang" control) to trigger individual pulses in a step-wise fashion, which can be useful for debugging purposes.

Third, there's the `arp` subpatch containing the guts of the arpeggiator itself. You can send a `panic` message to the arpeggiator to kill hanging notes, and switch the metronome on and off with the toggle control in the upper-right corner (saved in the patch). A gray slider gives an indication of the pulse strengths, and the gray bang control besides it briefly flashes at the beginning of each bar. You'll also notice that this bang control temporarily goes red if you stop the sequence, until the arpeggiator finishes playing the current bar. (If needed, you can also stop the sequence immediately using the start/stop toggle in the `time` subpatch.)
...
Then there's the `gain` control which sets the input gain of the *velocity tracker*, which can be enabled or disabled with the `veltrk` toggle (saved in the patch). The purpose of the velocity tracker is to calculate a kind of envelope from the velocities of the notes that you play and adjust the velocities of output notes generated by the arpeggiator accordingly, so that they follow your performance. The current envelope value is displayed in the number box to the right of the toggle, and the `gain` control, which is bound to the MIDI volume controller (CC7) and also stored in presets, allows you to boost that value if needed. This is a quick remedy if your MIDI controller produces very low velocities, as some controllers do, but you can also use it as an expression control.

Hidden away at the bottom of the `arp` subpatch you'll find two additional toggles, `bend` and `touch` (both saved in the patch), which determine whether to pass through pitch bend and aftertouch (a.k.a. channel pressure) messages from MIDI input to output. As shipped, pass-through is enabled for pitch bend and disabled for aftertouch, but you can change this to whatever you like if you open that subpatch.
