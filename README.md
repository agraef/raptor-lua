...
Using the patch is easy enough, however. Hook up your MIDI keyboard and synthesizer to Pd's MIDI input and output, respectively, press the green `play` button and just play some chords. I'm mostly using Raptor with a MIDI guitar and a [Nektar PACER][] foot pedal these days, so the patch is somewhat tailored for that use case, but it also works fine with just a MIDI keyboard and a free software synth such as [Qsynth][].

This project is still work in progress. In particular, the included presets need some work, but hey, you get them for free, so feel free to modify and use them for whatever purpose! ;-) Bug reports and other contributions are welcome. And I'm always curious about music made with my programs, so please do send me links to music produced with Raptor!
...
Finally, Raptor can also sync to an external time source via MIDI clock messages, in which case that time source takes over Raptor's transport and determines tempo and pulse period. Most DAWs support this, you just need to enable MIDI clock output in the DAW and connect it to any of Pd's MIDI inputs. As soon as Raptor starts receiving MIDI clocks, the pulse button in the `time` patch turns red to indicate MIDI clock sync, playback starts automatically, and the current tempo and pulse period are displayed in the tempo section. Playback stops as soon as Raptor receives a MIDI stop message, or hasn't received MIDI clocks for a little while (3 seconds in the current implementation, but this value can be changed in the `midiclock` abstraction).

**NOTE:** While MIDI sync should always work if your DAW supports it, pulses may appear to be out of phase if the meter settings don't match up, or if your DAW lacks support for song position pointer (SPP) messages and you start playback with an upbeat. Sometimes it may be fun to do this on purpose, but usually you'll want to make sure that you have meter and anacrusis set correctly to avoid such mishaps.
...
Version 6 of Raptor now finally has an integrated looper facility. There are two related GUI controls, a numbox-toggle pair labeled `loop`, at the bottom of the `panel` subpatch. The numbox lets you tell Raptor beforehand how many bars of output from the arpeggiator it should repeat once you engage the toggle. Turning on the toggle saves the loop in internal memory and immediately starts playing it back, turning it off again clears the loop from internal memory and resumes normal arpeggiator operation. Note that while the loop is playing, note input to the arpeggiator is suspended, so you can use your input controller to play along. Control input works normally, though, so you can also start twiddling the knobs, change presets, etc.

Another numbox-toggle pair labeled `load/save` can be found in the `loop` subpatch, which lets you save the loop that is currently playing to disk and reload it later. Once a loop has been recorded and is playing back, the pulse indicator in the upper-right corner of the subpatch will briefly flash at the start of each loop iteration, and the `load/save` button turns red to indicate that you can save the loop to a file by pressing it. The file is saved in the preset folder under the name of the preset that is currently selected, with the number in the numbox (0..99) and the extension `.loop` tacked onto it. Note that you can *only* save a loop while it is still playing! Once loop playback is stopped, the `load/save` button will be its normal white color again. You can now load the saved loop at any time (even after exiting and relaunching Raptor). Both the `loop` and the `load/save` button have bindings in the PACER configuration, so they can be operated while your hands remain on your input device.

The looper only kicks in once transport is rolling and the arpeggiator starts playing, and it always quantizes loops to entire bars, so you *must* play at least one bar before pressing the `loop` button outputs anything. (This kind of "launch quantization" should be familiar to Ableton Live users. It is there to ensure accurate results even if you fail to hit the bar boundaries exactly when pushing the button.) Also, depending on how many steps you played before pushing the `loop` button, the loop may actually be shorter than the number of bars specified in the `loop` numbox, which only denotes the *maximum* size of the loop.
...
In the second row you find the `trace` control (saved in the patch) which changes the amount of time that note-offs are delayed. This produces a kind of "legato" effect, making notes stick around a little longer while you're changing chords. The slider goes from 0 to 600 ms, the default being 300 ms (the button to the right of the slider resets the control to that value), setting it to zero disables the effect. There's also a `gain` control which sets the input gain of the *velocity tracker*, which can be enabled or disabled with the `veltrk` toggle (saved in the patch). The purpose of the velocity tracker is to calculate a kind of envelope from the velocities of the notes that you play and adjust the velocities of output notes generated by the arpeggiator accordingly, so that they follow your performance. The current envelope value is displayed in the number box to the right of the toggle, and can also be changed manually there if needed. Alternatively, the `gain` control, which is bound to the MIDI volume controller (CC7) and also stored in presets, allows you to boost the envelope value on the fly if needed. This is a quick remedy if your MIDI controller produces very low velocities, as some controllers do, or you could also use it as an expression control. But note that the `gain` value is just for amplification, not attenuation; changing the envelope value in the numbox goes both ways.
...
Obviously, it would be nice to have support for OSC to get around the limited resolution of MIDI CCs. But on the other hand OSC has its own quirks, and it's a lot easier to get connected with MIDI. So don't hold your breath. :)

It goes without saying that Raptor's looper is (by design) quite basic. Its main purpose is to give you a simple way of repeating a generated musical phrase while you have your hands free for soloing, diffusion (knob-twiddling), or getting that beautiful pattern recorded in a DAW before it vanishes forever. If you need more functionality, then I'd recommend combining Raptor with a DAW tailored to live usage, such as Ableton Live or Bitwig Studio, or even just a standard DAW like Ardour or Reaper. This will also give you the ability to record the *input* to the arpeggiator, which makes it much easier to tweak the results in post-production. We've done that many times with earlier Raptor versions, so I know that this is feasible (albeit more arduous).

That said, some usability improvements are still in order. In particular, in the heat of the battle it's all too easy to loose that precious loop that you previously recorded if you accidentally press loop load/save instead of the loop start/stop button. Also, at most 256 steps can be recorded in the current implementation, and quantization is limited to whole bars right now. Moreover, the velocity tracker levels aren't saved with the loops, so a loop may sound much louder or quieter next time you load it, but you can easily work around this issue by manually adjusting the tracker level.

Anything else that seems to be missing or not working properly? File a bug report, or (better yet) submit a pull request!
