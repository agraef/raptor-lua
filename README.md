...
Using the patch is easy enough, however. Hook up your MIDI keyboard and synthesizer to Pd's MIDI input and output, respectively, press `play` and just play some chords. I'm mostly using Raptor with a MIDI guitar and a [Nektar PACER][] foot pedal these days, so the patch is somewhat tailored for that use case, but it also works fine with just a MIDI keyboard and a free software synth such as [Qsynth][].
...
You can have a whole band of Raptors accompanying you, by running multiple instances of the main patch in concert in the same Pd instance. Just make sure that you select one of the instances as the time master (`M` toggle in the `time` subpatch), which happens automatically if you press the big green `play` toggle in one of the instances.

Talking about controls, most of them have MIDI bindings (see the "Control" section below) and are also recorded in presets, but there are a few GUI controls that you should know about. As indicated below, some of these are saved in the raptor6 patch itself, so you can just save the patch to change their defaults.
...
- *PC* refers to "program change", *CC* to "control change" messages. Like any MIDI input in Raptor, these are only received on the MIDI input channel (`in` numbox) set in the panel, which can be zero to denote "omni", i.e., receive on all MIDI channels.
- The table also lists the range of parameters. For "discrete" controls (0-1 switches: hold, raptor mode, and uniq; 0-n switches: mode), the control value is taken as is, and any value greater than the number of alternatives is clamped to the given range. Thus, for the 0-1 switches, *any* positive value means "on".
