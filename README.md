...
This project is still work in progress. There's some stuff on the TODO list, most notably OSC control. The included presets also need some work, but hey, you get them for free, and you can always change them! ;-) Bug reports and other contributions are welcome. And I'm always curious about music made with my programs, so please do send me links to music produced with Raptor!
...
Version 6 of Raptor now finally has an integrated looper facility. There are two related GUI controls, a numbox-toggle pair labeled `loop`, at the bottom of the `panel` subpatch. The numbox lets you tell Raptor beforehand how many past bars of output from the arpeggiator it should repeat once you engage the toggle. Turning on the toggle saves the loop in internal memory and immediately starts playing it back, turning it off again clears the loop from internal memory and resumes normal arpeggiator operation. Note that while the loop is playing, note input to the arpeggiator is suspended, so you can use your input controller to play along. Control input works normally, though, so you can also start twiddling the knobs, change presets, etc.

Another numbox-toggle pair labeled `load/save` can be found in the `loop` subpatch, which lets you save the loop that is currently playing to disk and reload it later. Once a loop has been recorded and is playing back, the `load/save` button turns red to indicate that you can save the loop to a file by pressing it. The file is saved in the preset folder under the name of the preset that is currently selected, with the number in the numbox (0..99) and the extension `.loop` tacked onto it. Note that you can *only* save a loop while it is still playing! Once loop playback is stopped, the `load/save` button will be its normal white color again. You can now load the saved loop at any time (even after exiting and relaunching Raptor) by making sure that you have the right loop number in the numbox and pressing the `load/save` button. Both the `loop` and the `load/save` button have bindings in the PACER configuration, so they can be operated while your hands remain on your input device.

It goes without saying that Raptor's looper has its limitations, but it's intentionally designed to be as basic as possible. Its main purpose is to give you a simple way of repeating a musical phrase, while you have your hands free for soloing, knob-twiddling, or maybe getting that awesome pattern recorded in a DAW before it vanishes forever. If you need more functionality, in particular the ability to record the *input* to the arpeggiator, which will give you much more possibilities for variation and diffusion, then I'd really recommend combining Raptor with a DAW tailored to live usage, such as Ableton Live or Bitwig Studio.
...
The looper still needs some usability improvements. In particular, in the heat of the battle it's all too easy to loose that precious loop that you previously recorded if you accidentally press loop load/save instead of the loop start/stop button. Also, at most 128 steps can be recorded in the current implementation. And the velocity tracker levels aren't saved with the loops, so a loop may sound much louder or quieter next time you load it, depending on the current level of the tracker. (But you can easily work around that by manually adjusting the tracker level.)
